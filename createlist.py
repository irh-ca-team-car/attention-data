...
for i in modes:
  if i[0:2] == 'DS':
    current_dir = os.path.join(opt.D,i,"images")
    file_full_path=os.path.join(opt.D,i,"full.txt")
    file_full = open(file_full_path, "w") 
    files = [x for x in os.listdir(current_dir) if x[-4::]==".jpg"]
    files.sort()
    for file in files:
    ...
    for sep in separations:
      ...
      for train_file in half:
        ...
        file_sep.write(os.path.realpath(current_dir + "/" + title + '.jpg') + "\n")
      for val_file in half2:
        title,ext= os.path.splitext(os.path.basename(val_file))
        file_sep2.write(os.path.realpath(current_dir + "/" + title + '.jpg') + "\n")
      file_sep.close()
    file_full.close()
